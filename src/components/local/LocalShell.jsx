...
export default function LocalShell({children,className=""}) {
  ...
  return <div className={`cp-app ${className}`}>
    ...
    <header className="cp-header"><Link className="cp-brand" to="/" aria-label="Click & Promo, accueil"><span className="cp-logomark">c<span>p</span><i/></span><span>click<span className="cp-brand-amp">&</span>promo<span className="cp-brand-dot">.</span></span></Link>
      ...
      <div className="cp-header-actions">{user?.role==="admin"&&<Link className="cp-account" to="/Admin" aria-label="Espace administration"><ShieldCheck size={18}/><span>Administration</span></Link>}<Link className="cp-pro-link" to="/Dashboard">Espace commerçant <ArrowUpRight size={15}/></Link><Link className="cp-account" to={user?"/Profil":"/Login"} aria-label={user?"Mon compte":"Se connecter"}><UserRound size={18}/><span>{user?"Mon compte":"Se connecter"}</span></Link></div>
    </header>
    ...
    <nav className="cp-mobile-nav" aria-label="Navigation mobile">{[[Compass,"Découvrir","/"],[Map,"Carte","/Carte"],[Heart,"Favoris","/Favoris"],user?.role==="admin"?[ShieldCheck,"Admin","/Admin"]:[Store,"Mon commerce","/Dashboard"],[UserRound,"Compte",user?"/Profil":"/Login"]].map(([Icon,label,path])=><NavLink key={label} to={path} end><Icon size={21}/><span>{label}</span></NavLink>)}</nav>
  </div>;
}